...
impl<H: ContainsHeap> HeapReader<'_, H> {
    ...
    fn with<R>(heap: &mut H, f: impl for<'a> FnOnce(&'a mut HeapReader<'a, H>) -> R) -> R {
        f(&mut HeapReader {
            heap: &mut *heap,
            phantom: PhantomData,
        })
...
impl<'a, H: ContainsHeap> HeapReader<'a, H> {
    ...
    fn read(&mut self, id: HeapId) -> HeapReadOutput<'a> {
        #[inline]
        fn heap_read<'a, T>(data: &mut T) -> HeapRead<'a, T> {
            HeapRead {
        ...
        let data = entry.data.as_mut().expect("HeapReader::read: data currently borrowed");

        match data {
            HeapData::Str(s) => HeapReadOutput::Str(heap_read(s)),
            HeapData::Bytes(bytes) => HeapReadOutput::Bytes(heap_read(bytes)),
            HeapData::List(list) => HeapReadOutput::List(heap_read(list)),
            HeapData::Tuple(tuple) => HeapReadOutput::Tuple(heap_read(tuple)),
            HeapData::NamedTuple(named_tuple) => HeapReadOutput::NamedTuple(heap_read(named_tuple)),
            HeapData::Dict(dict) => HeapReadOutput::Dict(heap_read(dict)),
            HeapData::Set(set) => HeapReadOutput::Set(heap_read(set)),
            HeapData::FrozenSet(frozen_set) => HeapReadOutput::FrozenSet(heap_read(frozen_set)),
            HeapData::Closure(closure) => HeapReadOutput::Closure(heap_read(closure)),
            HeapData::FunctionDefaults(function_defaults) => {
                HeapReadOutput::FunctionDefaults(heap_read(function_defaults))
            }
            HeapData::Cell(cell_value) => HeapReadOutput::Cell(heap_read(cell_value)),
            HeapData::Range(range) => HeapReadOutput::Range(heap_read(range)),
            HeapData::Slice(slice) => HeapReadOutput::Slice(heap_read(slice)),
            HeapData::Exception(simple_exception) => HeapReadOutput::Exception(heap_read(simple_exception)),
            HeapData::Dataclass(dataclass) => HeapReadOutput::Dataclass(heap_read(dataclass)),
            HeapData::Iter(monty_iter) => HeapReadOutput::Iter(heap_read(monty_iter)),
            HeapData::LongInt(l) => HeapReadOutput::LongInt(heap_read(l)),
            HeapData::Module(module) => HeapReadOutput::Module(heap_read(module)),
            HeapData::Coroutine(coroutine) => HeapReadOutput::Coroutine(heap_read(coroutine)),
            HeapData::GatherFuture(gather_future) => HeapReadOutput::GatherFuture(heap_read(gather_future)),
            HeapData::Path(path) => HeapReadOutput::Path(heap_read(path)),
        }
    }
}

enum HeapReadOutput<'a> {
    Str(HeapRead<'a, Str>),
...
impl<'a, T> HeapRead<'a, T> {
    fn get<'r>(&self, _: &'r HeapReader<'a, impl ContainsHeap>) -> &'r T {
        // SAFETY: (DH)
        ...
        //  - (FIXME) The type of the `HeapValue` can never change once allocated. This is
        //    guaranteed by lack of `decref` plus never handing out `&mut HeapData`.
        //    (Need to change `Heap::get_mut` to return a wrapper enum with e.g. `HeapValueMut::List(&mut List)`
        //     so that the `HeapValue` discriminant is inaccessible outside of this module.)
    ...
    fn get_mut<'r>(&mut self, _: &'r mut HeapReader<'a, impl ContainsHeap>) -> &'r mut T {
...
/// Example of how this `HeapReader` API can allow for concise two-phase borrowing which permits
/// safely interacting with heap data in an efficient matter while statically guaranteeing no
/// mutable aliasing.
fn list_inplace_add(src_list: HeapId, dest_list: HeapId, heap: &mut Heap<impl ResourceTracker>) {
    HeapReader::with(heap, |reader| {
        // Index into the heap for both types and get typed information out.
        let HeapReadOutput::List(src) = reader.read(src_list) else {
            panic!("Expected List");
        };
        let HeapReadOutput::List(mut dest) = reader.read(dest_list) else {
            panic!("Expected List");
        };
        ...
        let next = dest
            ...
            .as_slice()
            .iter()
            .chain(src.get(reader).as_slice()) // e.g. if this was `.get_mut`, the code would refuse to compile due to mutable borrow on reader
            .map(|i| i.clone_with_heap(reader.heap))
            .collect();

        std::mem::replace(dest.get_mut(reader).as_vec_mut(), next).drop_with_heap(reader.heap);
    });
}
